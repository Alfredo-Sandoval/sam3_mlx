...
def concat_padded_sequences(seq1, mask1, seq2, mask2, return_index: bool = False):
    ...
    concatenated_sequence = mx.take_along_axis(
        source_sequence, source_index, axis=0
    )
...
class Prompt:
    ...
    def append_masks(self, masks, labels=None, attn_mask=None):
        ...
        if self.mask_embeddings is None:
            ...
            if labels is None:
                self.mask_labels = mx.ones(
                    (mask_seq_len, bs), dtype=mx.int64
                )
            else:
                self.mask_labels = labels
            if attn_mask is None:
                self.mask_mask = mx.zeros(
                    (bs, mask_seq_len), dtype=mx.bool_
                )
            else:
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def __call__(self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None):
        ...
        if masks is not None and self.mask_encoder is None:
            raise NotImplementedError("Mask prompts require a configured mask_encoder.")
