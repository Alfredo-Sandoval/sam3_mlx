...
class SAM3VLBackbone(nn.Module):
    def __init__(
        ...
        super().__init__()
        if compile_visual:
            raise NotImplementedError("compile_visual is not implemented in MLX.")
        if act_ckpt_whole_vision_backbone or act_ckpt_whole_language_backbone:
            raise NotImplementedError(
                "Whole-backbone activation checkpointing is not implemented in MLX."
            )
    ...
    def forward(self, samples, captions, input_boxes=None, additional_text=None):
        return self(samples, captions, input_boxes, additional_text)

    def forward_image(self, samples: mx.array):
        return self.call_image(samples)
