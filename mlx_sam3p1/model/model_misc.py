...
class MultiheadAttentionWrapper(nn.MultiHeadAttention):
    ...
    def _to_additive_mask(mask):
        ...
        if mask.dtype == mx.bool_:
            return mx.where(mask, -float("inf"), 0.0)
        return mask

    def _normalize_attn_mask(self, mask, batch_size):
        mask = self._to_additive_mask(mask)
        if mask is None:
            return None
        if mask.ndim == 2:
    ...
    def _combine_masks(self, attn_mask, key_padding_mask, base_mask, queries):
        final_mask = self._normalize_attn_mask(base_mask, queries.shape[0])
        attn_mask = self._normalize_attn_mask(attn_mask, queries.shape[0])
        if key_padding_mask is not None:
            key_padding_mask = self._to_additive_mask(key_padding_mask)
            key_padding_mask = key_padding_mask[:, None, None, :]

        for mask in (attn_mask, key_padding_mask):
            if mask is not None:
                final_mask = mask if final_mask is None else final_mask + mask
        return final_mask

    @staticmethod
    def _pop_alias(kwargs, canonical, alias):
        ...
        if canonical_value is not None and alias_value is not None:
            raise TypeError(f"Received both {canonical!r} and {alias!r}.")
        return canonical_value if canonical_value is not None else alias_value

    def __call__(self, *args, **kwargs):
        queries = self._pop_alias(kwargs, "queries", "query")
        keys = self._pop_alias(kwargs, "keys", "key")
        values = self._pop_alias(kwargs, "values", "value")

        if len(args) > 3:
            raise TypeError(f"Expected at most 3 positional arrays, got {len(args)}.")
        if len(args) > 0:
            if queries is not None:
                raise TypeError("queries was provided both positionally and by keyword.")
            queries = args[0]
        if len(args) > 1:
            if keys is not None:
        ...
        mask = self._combine_masks(attn_mask, key_padding_mask, base_mask, queries)
        return super().__call__(queries, keys, values, mask=mask)
