...
from mlx_sam3p1.model.model_misc import Mlp, LayerScale, DropPath
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: mx.array) -> mx.array:
    ...
    if rel_pos.shape[0] != max_rel_dist:
        raise NotImplementedError(
            "Relative-position interpolation is not implemented in the MLX port."
        )
...
def concat_rel_pos(
    ...
    rel_pos_w: mx.array,
    rescale: bool = False,
    relative_coords: Optional[mx.array] = None,
) -> Tuple[mx.array, mx.array]:
    q_h, q_w = q_hw
    k_h, k_w = k_hw
    ...
    rel_h = mx.einsum("bhwc,hkc->bhwk", r_q, rh) * new_scale
    rel_w = mx.einsum("bhwc,wkc->bhwk", r_q, rw) * new_scale

    eye_h = mx.eye(k_h, dtype=q.dtype).reshape(1, k_h, 1, k_h)
    eye_w = mx.eye(k_w, dtype=q.dtype).reshape(1, 1, k_w, k_w)
    ...
    q = q.reshape(batch, q_h * q_w, -1)
    k = mx.concat([k.reshape(batch, k_h, k_w, -1), eye_h, eye_w], axis=-1)
    k = k.reshape(batch, k_h * k_w, -1)
    return q, k
...
class Attention(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if x.ndim == 4:
            ...
            B, L, _ = x.shape
            ndim = 3
            side = int(math.sqrt(L - s))
            assert side * side == L - s
            H = W = side
...
class ViT(nn.Module):
    def __init__(
        ...
        if compile_mode not in (None, False):
            raise NotImplementedError("ViT compile_mode is not implemented in MLX.")
    ...
    def __call__(self, x: mx.array) -> List[mx.array]:
        ...
        if self.retain_cls_token:
            x = mx.concat(
                [self.class_embedding, x.reshape(x.shape[0], -1, x.shape[-1])],
                axis=1,
            )
            s = 1
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if (i == self.full_attn_ids[-1]) or (
                ...
                if feats.ndim == 4:
                    ...
                    assert side * side == feats.shape[1]
                    h = w = side
                    feats = feats.reshape(
    ...
    def get_layer_id(self, layer_name: str) -> int:
        raise NotImplementedError("Layer id lookup is not implemented in the MLX port.")
