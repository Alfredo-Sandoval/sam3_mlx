...
class Transformer(nn.Module):
    def __init__(
        ...
        if compile_mode not in (None, False):
            raise NotImplementedError("Text Transformer compile_mode is not implemented.")
        self.width = width
        self.layers = layers
...
def text_global_pool(
    ...
    if pool_type == "first":
        ...
        pooled, tokens = x[mx.arange(x.shape[0]), mx.argmax(text, axis=-1)], x
    else:
        pooled = tokens = x
...
class VETextEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        if compile_mode not in (None, False):
            raise NotImplementedError("VETextEncoder compile_mode is not implemented.")
