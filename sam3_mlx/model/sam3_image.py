...
def _to_index_list(value) -> List[int]:
    return [int(item) for item in to_numpy(value).reshape(-1).tolist()]
...
class Sam3Image(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "multimask_otuput" in kwargs:
            multimask_output = kwargs.pop("multimask_otuput")
        if kwargs:
    ...
    def _get_img_feats(self, backbone_out, img_ids):
        ...
        if isinstance(img_batch, mx.array):
            ...
            image = mx.stack(
                [mx.array(img_batch[index]) for index in unique_ids],
                axis=0,
            )
    ...
    def _get_best_mask(self, out):
        prev_mask_idx = mx.argmax(out["pred_logits"], axis=1).squeeze(1)
        batch_idx = mx.arange(out["pred_logits"].shape[0], dtype=mx.int64)
        ...
        return prev_mask_pred.flatten(-2).transpose(2, 0, 1)
    ...
    def _postprocess_out(self, out: Dict, multimask_output: bool = False):
        num_mask_boxes = out["pred_boxes"].shape[1]
        if not self.training and multimask_output and num_mask_boxes > 1:
            ...
            best_mask_idx = mx.argmax(out["pred_logits"], axis=1).squeeze(1)
            batch_idx = mx.arange(best_mask_idx.shape[0], dtype=mx.int64)

            out["pred_logits"] = out["pred_logits"][batch_idx, best_mask_idx][:, None]
            if "pred_masks" in out:
                out["pred_masks"] = out["pred_masks"][batch_idx, best_mask_idx][:, None]
            out["pred_boxes"] = out["pred_boxes"][batch_idx, best_mask_idx][:, None]
            out["pred_boxes_xyxy"] = out["pred_boxes_xyxy"][batch_idx, best_mask_idx][
                :, None
            ]
        return out
    ...
    def back_convert(self, targets):
        return {
            "boxes": targets.boxes.reshape(-1, 4),
            "boxes_xyxy": box_cxcywh_to_xyxy(targets.boxes.reshape(-1, 4)),
            "boxes_padded": targets.boxes_padded,
            "positive_map": mx.ones((len(targets.boxes), 1), dtype=targets.boxes.dtype),
    ...
    def predict_inst(
        ...
        **kwargs,
        ...
        predictor._orig_hw = [(orig_h, orig_w)]
        try:
    ...
    def predict_inst_batch(
        ...
        **kwargs,
        ...
        batch_size = len(orig_heights)
        if batch_size != len(orig_widths):
        ...
        if features["image_embed"].shape[0] != batch_size:
            raise AssertionError(
                ...
                f"{features['image_embed'].shape[0]}, {len(orig_heights)}, "
                f"{len(orig_widths)}"
            )
        predictor._features = features
        predictor._is_image_set = True
...
class Sam3ImageOnVideoMultiGPU(Sam3Image):
    ...
    def forward_video_grounding_multigpu(self, *args, **kwargs):
        del args, kwargs
        _raise_image_unsupported(
