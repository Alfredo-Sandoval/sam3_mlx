...
class Sam3Processor:
    ...
    def set_image_batch(self, images: List[np.ndarray], state=None):
        raise NotImplementedError(
            "set_image_batch is not supported in the MLX port yet; use "
            "set_image for a single image. Batch parity with "
            "facebookresearch/sam3 requires broader model support."
        )
    ...
    def _call_grounding(self, state: Dict):
        if "original_heights" in state or "original_widths" in state:
            raise NotImplementedError(
                "Batch grounding is not supported in the MLX port yet; use "
                "set_image for a single image."
            )
