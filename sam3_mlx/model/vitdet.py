...
def get_abs_pos(
    ...
    if size != h or size != w:
        ...
        if tiling:
            ...
            new_abs_pos = upsample_fn(new_abs_pos.transpose(0, 2, 3, 1)).transpose(
                0, 3, 1, 2
            )
...
class Attention(nn.Module):
    ...
    def _apply_rope(
        ...
        freqs_cis = self._rope_freqs_for_tokens(q.shape[-2], spatial_size)
...
class ViT(nn.Module):
    ...
    def _init_weights(self, m: nn.Module) -> None:
        if isinstance(m, nn.Linear):
            m.weight = (
                mx.random.truncated_normal(lower=-2, upper=2, shape=m.weight.shape)
                * 0.02
            )
