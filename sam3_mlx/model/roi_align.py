...
def check_roi_boxes_shape(
    boxes: Union[mx.array, Union[list[mx.array], tuple[mx.array]]],
):
    if isinstance(boxes, (list, tuple)):
        for _array in boxes:
            assert _array.shape[1] == 4, (
...
def _roi_align(
    ...
    roi_bin_grid_h = (
        sampling_ratio if exact_sampling else mx.ceil(roi_height / pooled_height)
    )
    roi_bin_grid_w = (
        sampling_ratio if exact_sampling else mx.ceil(roi_width / pooled_width)
    )
