...
class TransformerEncoder(nn.Module):
    ...
    def _prepare_multilevel_features(self, srcs, masks, pos_embeds):
        ...
        mask_flatten = mx.concat(mask_flatten, axis=1) if has_mask else None # bs, \sum{hxw}
        lvl_pos_embed_flatten = mx.concat(lvl_pos_embed_flatten, axis=1) # bs, \sum{hxw}, c
        ...
        **kwargs,
        ...
        if compile_mode not in (None, False):
            raise NotImplementedError(
                "TransformerEncoderFusion compile_mode is not implemented in MLX."
            )
        super().__init__(
            layer,
            num_layers,
