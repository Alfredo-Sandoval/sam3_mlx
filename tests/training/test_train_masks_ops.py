...
def test_decode_coco_rle_accepts_compressed_and_uncompressed_counts():
    ...
    assert normalize_rle_counts("132") == [1, 3, 2]
    np.testing.assert_array_equal(decode_coco_rle(compressed), expected)
    np.testing.assert_array_equal(decode_coco_rle(uncompressed), expected)
...
def test_ann_to_rle_compresses_uncompressed_counts_and_preserves_compressed():
    ...
    compressed_input = {"size": [2, 3], "counts": "132", "area": 3}
    already_compressed = ann_to_rle(compressed_input, im_info)

    assert from_uncompressed == {"size": [2, 3], "counts": "132"}
    assert already_compressed is compressed_input
