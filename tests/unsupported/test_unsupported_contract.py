...
def test_upstream_md_unsupported_section_is_in_sync_with_registry():
    ...
    UPSTREAM.md is intentionally local/gitignored in the public tree. When the
    ...
    if not upstream_md.exists():
        pytest.skip(
            "UPSTREAM.md is not present (local/gitignored provenance doc); "
            "generate with python -m sam3_mlx._unsupported after creating "
            "a scaffold that includes the AUTOGENERATED markers."
        )
